...
func RowsScan(rows *sql.Rows, v interface{}) (int, error) {
...
// else < 0 Not limited
func RowsLimit(rows *sql.Rows, limit int) ([]string, [][][]byte, error) {
	if limit == 0 {
		return nil, nil, nil
	}
	if !rows.Next() {
...
func DataScan(key []string, data [][][]byte, v interface{}) error {
	if len(data) == 0 || len(key) == 0 {
		return nil
	}

	val := reflect.ValueOf(v)
	return rowsScanValues(key, data, val)
}

// rowsScanValues rows scan values
func rowsScanValues(key []string, data [][][]byte, val reflect.Value) error {
	switch val.Kind() {
	default:
	...
	case reflect.Ptr:
		...
		return rowsScanValues(key, data, val.Elem())
	case reflect.Slice:
		l := len(data)
		val.Set(reflect.MakeSlice(val.Type(), l, l))
		fallthrough
	case reflect.Array:
		return rowsScanValue(key, data, val)
	case reflect.Struct:
		key0 := colAdjust(val.Type(), key)
...
func rowsScanValue(key []string, data [][][]byte, val reflect.Value) error {
	tt := val.Type().Elem()
	ps := 0
...
func colAdjust(tt reflect.Type, key []string) [][]string {
	m := map[string]int{}
	for i, v := range key {
		m[v] = i
	}
	rk := make([][]string, len(key))
	colAdjustMap(tt, key, nil, m, rk)
	return rk
}

// colAdjustMap Adjust col map
func colAdjustMap(tt reflect.Type, key []string, prefix []string, m map[string]int, rk [][]string) bool {
	b := false
	nf := tt.NumField()
	for i := 0; i != nf; i++ {
		fi := tt.Field(i)
		tv := fi.Tag.Get(TagGrmSql)
		...
		if fi.Anonymous && fi.Type.Kind() == reflect.Struct &&
			colAdjustMap(fi.Type, key, append(prefix, fi.Name), m, rk) {
			continue
		}
		...
		if tv0 == "" {
			tv0 = Hump2Snake(fi.Name)
		}
		if k, ok := m[tv0]; ok && len(rk[k]) == 0 {
